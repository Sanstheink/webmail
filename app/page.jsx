...
export default function Webmail() {
  ...
  // 🌟 ชุดสีและพื้นหลังที่อัปเกรดแล้ว
  const bgApp = isDark 
    ? 'bg-gradient-to-br from-slate-950 via-[#0a0f1d] to-indigo-950/80 bg-animate' 
    : 'bg-gradient-to-br from-[#e0f2fe] via-[#f0f9ff] to-[#e0e7ff] bg-animate';
  
  const textMain = isDark ? 'text-white' : 'text-slate-900';
  const textSub = isDark ? 'text-slate-400' : 'text-slate-500';
  const glassPanel = isDark ? 'bg-slate-900/40 border-slate-700/50' : 'bg-white/40 border-white/60';
  const innerPanel = isDark ? 'bg-slate-900/70 border-slate-700/50' : 'bg-white/70 border-white/60';
  const hoverClass = isDark ? 'hover:bg-slate-800/60' : 'hover:bg-white/60';

  // สีของลูกแก้ว (Blobs) ที่ลอยอยู่ด้านหลัง
  const blob1 = isDark ? 'bg-indigo-600/20' : 'bg-cyan-300/40';
  const blob2 = isDark ? 'bg-fuchsia-700/20' : 'bg-purple-300/40';
    ...
    <div className={`min-h-screen relative flex items-center justify-center p-2 sm:p-6 overflow-hidden transition-all duration-700 ${bgApp}`}>
      ...
      {showSettings && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 backdrop-blur-md p-4 animate-[fadeIn_0.2s_ease-out]">
          <div className={`w-full max-w-md p-8 rounded-[32px] shadow-2xl relative transition-all duration-300 transform scale-100 ${isDark ? 'bg-slate-800/90 border-slate-700 text-white' : 'bg-white/90 border-white text-slate-900'} border backdrop-blur-xl`}>
            ...
            <div className="flex flex-col items-center mb-8">
              <div 
                ...
                className={`w-24 h-24 rounded-full cursor-pointer relative group overflow-hidden shadow-lg mb-3 flex items-center justify-center border-4 ${isDark ? 'bg-slate-700 border-slate-600' : 'bg-slate-200 border-white'}`}
              >
            ...
            <div className="space-y-4">
              <div className={`p-4 rounded-2xl flex justify-between items-center transition-colors ${isDark ? 'bg-slate-900/50' : 'bg-slate-50'}`}>
                <span className="font-semibold text-sm">Theme</span>
                <div className="flex bg-slate-500/20 rounded-full p-1 gap-1">
                  ...
                  <button onClick={() => updateSettings('dark', lang)} className={`px-4 py-1.5 text-xs font-bold rounded-full transition-all ${isDark ? 'bg-slate-800 shadow-md text-white scale-105' : 'text-slate-500 hover:text-slate-600'}`}>Dark</button>
                </div>
              </div>
              <div className={`p-4 rounded-2xl flex justify-between items-center transition-colors ${isDark ? 'bg-slate-900/50' : 'bg-slate-50'}`}>
                <span className="font-semibold text-sm">Language</span>
                <div className="flex bg-slate-500/20 rounded-full p-1 gap-1">
                  <button onClick={() => updateSettings(theme, 'en')} className={`px-4 py-1.5 text-xs font-bold rounded-full transition-all ${lang === 'en' ? 'bg-white shadow-md text-slate-900 scale-105' : 'text-slate-400 hover:text-slate-300'}`}>EN</button>
                  <button onClick={() => updateSettings(theme, 'th')} className={`px-4 py-1.5 text-xs font-bold rounded-full transition-all ${lang === 'th' ? 'bg-white shadow-md text-slate-900 scale-105' : 'text-slate-500 hover:text-slate-600'}`}>TH</button>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className={`w-full max-w-[1600px] h-[92vh] ${glassPanel} backdrop-blur-2xl rounded-[32px] shadow-[0_8px_32px_rgb(0,0,0,0.12)] border flex overflow-hidden p-2 relative z-10 gap-2 transition-all duration-500`}>
        ...
        {/* --- 1. DARK SIDEBAR --- */}
        <div className="w-16 bg-[#1A1A1A] rounded-[24px] flex flex-col items-center py-6 justify-between shrink-0 shadow-xl relative z-20 h-full">
          ...
          <div className="flex flex-col gap-5 items-center w-full">
            ...
            <button onClick={() => setShowSettings(true)} className="w-10 h-10 rounded-full bg-slate-700 flex items-center justify-center hover:scale-110 hover:shadow-[0_0_15px_rgba(255,255,255,0.2)] transition-all overflow-hidden shadow-lg border-2 border-[#1A1A1A]" title="โปรไฟล์">
            ...
            {/* Settings Button */}
            <button onClick={() => setShowSettings(true)} className="w-10 h-10 rounded-xl text-slate-400 hover:text-white hover:bg-white/10 hover:scale-110 transition-all flex justify-center items-center" title="การตั้งค่า">
            ...
            {/* Logout Button */}
            <button onClick={handleLogout} className="w-10 h-10 rounded-xl text-slate-400 hover:text-red-400 hover:bg-white/10 hover:scale-110 transition-all flex justify-center items-center" title="ออกจากระบบ">
        ...
        <div className="w-48 flex flex-col pt-6 px-3 shrink-0">
          ...
          <div className="flex flex-col gap-1">
            <button onClick={() => setCurrentFolder('inbox')} className={`group flex items-center justify-between px-3 py-2.5 rounded-xl text-sm transition-all duration-300 ${currentFolder === 'inbox' ? `${isDark ? 'bg-slate-800' : 'bg-white shadow-md'} font-bold ${textMain} translate-x-1` : `font-medium ${textSub}${hoverClass}`}`}>
            ...
            <button onClick={() => setCurrentFolder('sent')} className={`group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-300 ${currentFolder === 'sent' ? `${isDark ? 'bg-slate-800' : 'bg-white shadow-md'} font-bold ${textMain} translate-x-1` : `font-medium ${textSub}${hoverClass}`}`}>
            ...
            <button onClick={() => setCurrentFolder('drafts')} className={`group flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-300 ${currentFolder === 'drafts' ? `${isDark ? 'bg-slate-800' : 'bg-white shadow-md'} font-bold ${textMain} translate-x-1` : `font-medium ${textSub}${hoverClass}`}`}>
        ...
        <div className={`w-[320px] lg:w-[380px] ${glassPanel} backdrop-blur-xl rounded-[24px] flex flex-col overflow-hidden shrink-0 transition-colors duration-500`}>
          <div className={`p-5 border-b ${isDark ? 'border-slate-700/50' : 'border-white/40'}`}>
            <div className="flex justify-between items-center mb-5">
              <h2 className={`text-[22px] font-bold ${textMain} capitalize transition-colors`}>{t[currentFolder] || currentFolder}</h2>
              <button onClick={() => { resetCompose(); setIsComposing(true); }} className="w-8 h-8 rounded-full bg-[#1A1A1A] text-white flex items-center justify-center hover:bg-black hover:scale-110 transition-all shadow-md">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            {filteredEmails.length === 0 ? (
              ...
              filteredEmails.map((mail) => {
                ...
                return (
                  <div key={mail.id} onClick={() => handleEmailClick(mail)} className={`group relative p-5 border-b ${isDark ? 'border-slate-700/30' : 'border-white/20'} cursor-pointer transition-all duration-300 ${isSelected ? `${isDark ? 'bg-slate-800 shadow-md border-transparent' : 'bg-white shadow-md border-transparent'} rounded-xl m-2 scale-[1.02]` : hoverClass}`}>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center mb-1">
                        ...
                        <span className={`text-[12px] font-medium transition-colors ${isSelected ? (isDark ? 'text-indigo-400' : 'text-blue-500') : 'text-slate-400'} shrink-0`}>{new Date(mail.created_at).toLocaleTimeString('en-US', { hour: '2-digit', minute:'2-digit' })}</span>
                      </div>
        ...
        <div className={`flex-1 ${innerPanel} backdrop-blur-xl rounded-[24px] flex flex-col overflow-hidden relative z-0 transition-colors duration-500 shadow-inner`}>
          {isComposing ? (
            <div className="h-full flex flex-col animate-[fadeIn_0.4s_ease-out]">
              <div className={`px-8 py-5 border-b flex justify-between items-center ${isDark ? 'border-slate-700/50' : 'border-slate-200/50'}`}>
              ...
              <div className="p-8 flex-1 overflow-y-auto custom-scrollbar">
                <form onSubmit={handleSendEmail} className="flex flex-col h-full gap-4 max-w-3xl mx-auto">
                  <div className={`flex items-center border-b pb-2 transition-colors ${isDark ? 'border-slate-700/50 focus-within:border-indigo-500' : 'border-slate-200/50 focus-within:border-blue-500'}`}>
                  ...
                  <textarea value={body} onChange={(e) => setBody(e.target.value)} className={`flex-1 w-full bg-transparent p-2 focus:outline-none resize-none mt-4 leading-relaxed ${textMain} placeholder-slate-400/50`} placeholder="..."></textarea>
                  <div className={`flex justify-between items-center pt-4 border-t mt-auto ${isDark ? 'border-slate-700/50' : 'border-slate-200/50'}`}>
                    <div className="flex gap-2">
                      <div className="relative group">
                        <button type="button" className={`flex items-center gap-2 text-sm font-medium transition-colors px-4 py-2 rounded-full shadow-sm hover:scale-105 ${isDark ? 'bg-slate-700 text-slate-200 hover:bg-slate-600' : 'bg-white text-slate-600 hover:bg-slate-50'}`}>{t.attach}</button>
                    ...
                    <div className="flex gap-3">
                      <button type="button" onClick={handleSaveDraft} className={`px-6 py-2.5 rounded-full text-sm font-semibold transition-all hover:scale-105 ${isDark ? 'bg-slate-700 text-slate-300 hover:bg-slate-600' : 'bg-slate-200 text-slate-700 hover:bg-slate-300'}`}>{t.save}</button>
                      <button type="submit" disabled={sending} className="bg-[#1A1A1A] text-white px-8 py-2.5 rounded-full text-sm font-semibold hover:bg-black hover:scale-105 transition-all disabled:opacity-50 shadow-md">{t.send}</button>
                    </div>
                  </div>
                </form>
              </div>
            ...
            <div className="h-full flex flex-col animate-[fadeIn_0.4s_ease-out]">
              <div className={`px-8 py-4 border-b flex items-center gap-6 text-sm font-semibold transition-colors ${isDark ? 'bg-slate-800/50 border-slate-700/50 text-slate-300' : 'bg-white/30 border-slate-200/50 text-slate-600'}`}>
              ...
              <div className="p-10 overflow-y-auto flex-1 custom-scrollbar">
                <div className="flex items-start gap-4 mb-10">
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-lg shrink-0 overflow-hidden shadow-sm ${isDark ? 'bg-slate-700 text-white' : 'bg-slate-200 text-slate-700'}`}>
            ...
            <div className={`h-full flex flex-col items-center justify-center ${textSub} animate-[fadeIn_0.4s_ease-out]`}>
              <div className={`w-24 h-24 rounded-full mb-6 flex items-center justify-center shadow-inner ${isDark ? 'bg-slate-800/50' : 'bg-slate-100/50'}`}>
                <svg className={`w-10 h-10 ${isDark ? 'text-slate-600' : 'text-slate-300'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
              </div>
      ...
      {/* 🌟 CSS Keyframes สำหรับแอนิเมชันขั้นสูง */}
      <style dangerouslySetInnerHTML={{__html: `
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb { background-color: ${isDark ? '#475569' : 'rgba(0,0,0,0.1)'}; border-radius: 10px; }
      `}} />
    </div>
  );
}
